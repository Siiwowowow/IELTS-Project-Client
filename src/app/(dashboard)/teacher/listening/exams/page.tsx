...
export default function MyListeningExamsPage() {
  ...
  const {
    ...
    queryKey: ["teacher-listening-exams"],
    queryFn: () => listeningService.getAllExams({ myExams: true }),
  });

  // Delete mutation
  const deleteMutation = useMutation({
    mutationFn: (id: string) => listeningService.deleteExam(id),
    onSuccess: () => {
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto w-full p-4 md:p-6 font-sans">
      {/* Top Banner / Header */}
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-r from-slate-900 via-[#1B3A6B] to-slate-950 p-6 md:p-8 text-white shadow-xl">
        <div className="absolute right-0 top-0 -mr-16 -mt-16 w-64 h-64 bg-blue-500/10 rounded-full blur-3xl"></div>
        ...
        <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-blue-500/20 border border-blue-400/30 text-xs font-bold text-blue-200 uppercase tracking-widest">
              <IconSparkles size={12} className="text-amber-400" />
              <span>Instructor Portal</span>
            </div>
            <h1 className="text-3xl font-black tracking-tight">Manage Listening Exams</h1>
            <p className="text-blue-100/80 text-sm max-w-xl font-medium">
              Create and manage your IELTS Academic Listening simulation tests. Upload audio files, submit script transcripts, and sync question schemas.
            </p>
          </div>

          <Link
            href="/teacher/listening/create"
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold text-gray-800 flex items-center gap-2">
            <IconNotebook className="text-[#1B3A6B]" size={20} />
            <span>Your Listening Exams ({exams.length})</span>
          </h2>
        </div>
          ...
          <div className="flex items-center gap-4 p-5 bg-rose-50 border border-rose-100 rounded-2xl text-rose-800">
            ...
            <div>
              ...
              <p className="text-xs text-rose-700/80 mt-0.5">
                We encountered an error loading your listening exams. Please refresh the page.
              </p>
            </div>
          </div>
        )}
        ...
        {!isLoading && !isError && exams.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 bg-white rounded-2xl border border-gray-200 text-center p-6 gap-4">
            <div className="h-16 w-16 rounded-2xl bg-blue-50 border border-blue-100 text-[#1B3A6B] flex items-center justify-center shadow-sm">
              <IconFileText size={28} />
            </div>
            <div className="max-w-xs space-y-1">
              <p className="font-bold text-gray-800">No Exams Found</p>
              <p className="text-xs text-gray-500 font-medium">
                You haven't created any IELTS Listening mock exams yet. Start by crafting your first one!
              </p>
            </div>
            <Link
              href="/teacher/listening/create"
              className="px-4.5 py-2.5 rounded-lg bg-[#1B3A6B] hover:bg-[#152e54] text-white text-xs font-bold shadow-sm hover:shadow-md transition duration-200"
            >
              Get Started
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {exams.map((exam) => {
              ...
              return (
                <div
                  key={exam.id}
                  className="group relative flex flex-col justify-between rounded-2xl bg-white border border-gray-200 p-5 shadow-sm hover:shadow-md hover:border-blue-100 transition-all duration-350 min-h-[220px]"
                >
                  <div className="space-y-3">
                    ...
                    <div className="space-y-1">
                      <h3 className="font-black text-gray-900 group-hover:text-[#1B3A6B] transition-colors text-base truncate">
                        {exam.title}
                      </h3>
                    ...
                    <div className="flex items-center gap-4 py-2 border-y border-gray-50 text-xs font-semibold text-gray-600">
                      <div className="flex items-center gap-1">
                        <IconClock size={14} className="text-[#1B3A6B]" />
                  ...
                  <div className="mt-4 pt-3 border-t border-gray-50 flex items-center justify-between gap-2">
                    {isConfirmingDelete ? (
                      ...
                      <>
                        <Link
                          href={`/practice/listening/${exam.id}`}
                          target="_blank"
                          className="inline-flex items-center gap-1 px-3 py-2 text-xs font-bold text-[#1B3A6B] hover:text-[#152e54] hover:bg-blue-50/50 rounded-lg transition duration-200"
                        >
                          <IconEye size={14} />
                          <span>Preview</span>
                        </Link>
                        ...
                        <div className="flex gap-1">
                          <button
                            onClick={() => router.push(`/teacher/listening/create?edit=${exam.id}`)}
                            className="p-2 text-gray-500 hover:text-[#1B3A6B] hover:bg-blue-50/50 rounded-lg transition"
                            title="Edit Exam"
                          >
                            <IconEdit size={16} />
                          </button>
