      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-gray-900 flex items-center gap-2.5 uppercase tracking-tight">
            <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-rose-500 text-white shadow-md">
              <IconMicrophone size={24} />
            </span>
            Speaking Practice
          </h1>
          <p className="text-sm font-medium text-gray-500 mt-2 ml-12">
            Simulate real computer-based IELTS Speaking mock exams under official test timing constraints.
          </p>
        </div>
      </div>
      ...
      <section>
        {examsQuery.isLoading && (
          <div className="flex items-center justify-center py-20">
            <IconLoader2 size={32} className="animate-spin text-rose-500" />
          </div>
        )}
        ...
        {examsQuery.isError && (
          <div className="flex items-center gap-3 p-4 bg-red-50 border border-red-100 rounded-xl text-red-700 font-medium">
            ...
            <p className="text-sm">Failed to load speaking exams. Please refresh and try again.</p>
          </div>
        )}
        ...
        {!examsQuery.isLoading && !examsQuery.isError && exams.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-gray-400 gap-3">
            ...
            <p className="text-sm font-medium">No speaking exams available yet.</p>
          </div>
        )}

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {exams.map((exam) => (
            <SpeakingExamListCard key={exam.id} exam={exam} />
          ))}
        </div>
      </section>
    </div>
